...
<a href="<?php echo base_url(); ?>factors/lead_to_member" class="btn btn-primary btn-sm btn-tlhr-eval">Add Evaluation Factor</a>
	...
	<table class="table table-striped">
	  ...
	  <tbody>
	  	<?php $i=1; foreach ($factors_list as $key => $value): ?>
	  		<tr>
	  			<td><?php echo $i; ?></td>
	  			<td><?php echo @$value['tlm_cat_name']; ?></td>
	  			<td><?php echo @$value['tlm_eval_description']; ?></td>
	  			<td class="actions">
	  			<a class="btn btn-info btn-xs" href="<?php echo base_url(); ?>factors/lead_to_member/<?php echo @$value['tlm_eval_fact_id']; ?>">Edit</a>
	  			<a class="btn btn-danger btn-xs" onclick="javascript:deleteConfirm('<?php echo base_url().'factors/delete_lead_to_member/'.@$value['tlm_eval_fact_id'].'/'.@$value['tlm_cat_id'];?>');" deleteConfirm href="#">Delete</a></td>
	  		</tr>
	  	<?php $i++; endforeach ?>
	  </tbody>
	</table>
