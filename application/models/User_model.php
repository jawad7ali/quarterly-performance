...
class User_model extends CI_Model{
	...
	public function update_user_status($id,$status){
		...
		$this->db->update('qpe_user', array('status' => $status));
	...
	public function get_submited_members_total_dash($ids,$quarter){
		$allids = implode("','", explode(',', $ids));
		//echo "SELECT COUNT(DISTINCT qpe_evaluation.user_id) as totalsubmited FROM `qpe_evaluation` INNER JOIN qpe_user ON qpe_evaluation.user_id=qpe_user.User_id WHERE QUARTER(DATE_ADD(created_date,INTERVAL -60 DAY)) = ". $quarter ." and qpe_evaluation.user_id IN ('".$allids."') ";
		$query = $this->db->query("SELECT COUNT(DISTINCT qpe_evaluation.user_id) as totalsubmited FROM `qpe_evaluation` INNER JOIN qpe_user ON qpe_evaluation.user_id=qpe_user.User_id WHERE QUARTER(DATE_ADD(created_date,INTERVAL -60 DAY)) = ". $quarter ." and qpe_evaluation.user_id IN ('".$allids."') ");
		return $query->row();
	}
}